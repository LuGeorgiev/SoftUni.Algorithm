using System;
using System.Linq;
using System.Collections.Generic;
using Wintellect.PowerCollections;

namespace DijkstraAlgorithm
{
    //Prim starts from single node and buids
    //Kruskal starts with all node separeted and runs greedy to build the Spanning Tree Forest
    class Edge
    {
        public int First { get; set; }

        public int Second { get; set; }

        public int Weight { get; set; }
    }

    class Program
    {
        static HashSet<int> spanningTree = new HashSet<int>();
        static Dictionary<int, List<Edge>> nodeToEdges = new Dictionary<int, List<Edge>>();

        static void Main(string[] args)
        {
            var graph = new List<Edge>()
            {
                new Edge{ First=1,Second=2, Weight =4},
                new Edge{ First=1,Second=3, Weight =5},
                new Edge{ First=4,Second=5, Weight =8},
                new Edge{ First=3,Second=5, Weight =7},
                new Edge{ First=2,Second=4, Weight =2},
                new Edge{ First=1,Second=4, Weight =9},
                new Edge{ First=8,Second=9, Weight =7},
                new Edge{ First=5,Second=6, Weight =12},
                new Edge{ First=3,Second=4, Weight =20},
                new Edge{ First=7,Second=9, Weight =10},
            };

            var nodes = graph
                .Select(e => e.First)
                .Union(graph.Select(e => e.Second))
                .OrderBy(e => e)
                .ToHashSet();

            foreach (var edge in graph)
            {
                if (!nodeToEdges.ContainsKey(edge.First))
                {
                    nodeToEdges[edge.First] = new List<Edge>();
                }
                if (!nodeToEdges.ContainsKey(edge.Second))
                {
                    nodeToEdges[edge.Second] = new List<Edge>();
                }

                nodeToEdges[edge.First].Add(edge);
                nodeToEdges[edge.Second].Add(edge);
            }

            var distance = new int[nodes.Max() + 1];
            for (int i = 0; i < distance.Length; i++)
            {
                distance[i] = int.MaxValue;
            }

            //first node
            distance[1] = 0;

            //use outer reference for oredring
            var queue = new SortedSet<int>(
                    Comparer<int>.Create((f, s) => distance[f] - distance[s]));
            queue.Add(nodes.First());

            while (queue.Count!=0)
            {
                var min = queue.Min;
                queue.Remove(min);

                foreach (var edges in nodeToEdges[min])
                {
                    var otherNode = edges.First == min ? edges.Second : edges.First;
                    if (distance[otherNode] == int.MaxValue)
                    {
                        queue.Add(otherNode);
                    }
                    var newDistance = distance[min] + edges.Weight;
                    if (newDistance<distance[otherNode])
                    {
                        distance[otherNode] = newDistance;
                        // next two lines are needed to REOREDR the priority queue other approach is to initialize again
                        queue.Remove(otherNode);
                        queue.Add(otherNode);
                    }
                }
            }
            //as result we have all the weights to each node have to implement prev[] if path is needed
        }
    }
}
